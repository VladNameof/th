...
return null}}}
function t1032_unifyHeights(recId){var rec=document.querySelector('#rec'+recId);if(!rec)return;var container=rec.querySelector(' .t1032 .t-container');if(!container)return;var allRows=container.querySelectorAll('.t1032__row');for(var i=0;i<allRows.length;i++){var allTitles=allRows[i].querySelectorAll('.t1032__title');var allDescriptions=allRows[i].querySelectorAll('.t1032__descr');var allWrappers=allRows[i].querySelectorAll('.t1032__wrapper');var highestTitle=0;var highestDescription=0;var highestWrapper=0;Array.prototype.forEach.call(allTitles,function(title){title.style.height='auto';var titleDimension=t1032_getElementDimension(title);if(titleDimension.height>highestTitle){highestTitle=titleDimension.height}});if(window.innerWidth>=960&&t1032_isVisible(container)){Array.prototype.forEach.call(allTitles,function(title){title.style.height=highestTitle+'px'})}else{Array.prototype.forEach.call(allTitles,function(title){title.style.height='auto'})}
...
function t694_init(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var container=rec.querySelector('.t694');if(!container)return;t694_setHeight(recId);setTimeout(function(){t694_setHeight(recId)},500);window.addEventListener('resize',t_throttle(function(){t694_setHeight(recId)}));container.addEventListener('displayChanged',function(){t694_setHeight(recId)})}
...
function t850_init(recId){var rec=document.getElementById('rec'+recId);if(!rec)return;var wrapper=rec.querySelector('.t850');if(!wrapper)return;t_onFuncLoad('t_cards__moveClickOnCard',function(){t_cards__moveClickOnCard(recId)});t850_unifyHeights(recId);window.addEventListener('resize',t_throttle(function(){if(window.noAdaptive&&window.noAdaptive===!0&&window.isMobile)return;t850_unifyHeights(recId)}));window.addEventListener('load',function(){t850_unifyHeights(recId)});if(typeof jQuery!=='undefined'){$(wrapper).on('displayChanged',function(){t850_unifyHeights(recId)})}else{wrapper.addEventListener('displayChanged',function(){t850_unifyHeights(recId)})}
